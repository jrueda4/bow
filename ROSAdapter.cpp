...
#include <std_msgs/Int8.h>	//SIPI - added for reasons
...
ros::Publisher dropTrafficPublisher;
...
ros::Subscriber dropTrafficSubscriber;
...
void dropTrafficHandler(const std_msgs::Int8& msg);

//SIPI
int traffic = 0;		//keeps current traffic
int trafficAdjustment = 0;	//changes to current traffic
bool shouldBroadcast = false;
std_msgs::Int8 traffic_msg;	//used for passing messages to the GUI
void updateTraffic();
vector <Tag> tags; 		//SWRA: Broadcast after moving away from cluster
Point saved_loc;		//ditto
int unLocky = 0;		//ditto
Point initial;			//SWRA: to help translate locations
...
int main(int argc, char **argv) {
  ...
  dropTrafficSubscriber = mNH.subscribe("/dropoff_Traffic", 10, dropTrafficHandler);
  ...
  dropTrafficPublisher = mNH.advertise<std_msgs::Int8>("/dropoff_Traffic", 10, true);					//publishes how many swarmies are dropping cubes at center
...
void behaviourStateMachine(const ros::TimerEvent&)
{
 ...
 //TODO: this just sets center to 0 over and over and needs to change
      Point centerOdom;
      centerOdom.x = 0; //1.0 * cos(currentLocation.theta);
      centerOdom.y = 0; //1.0 * sin(currentLocation.theta);
      centerOdom.theta = centerLocation.theta;
      logicController.SetCenterLocationOdom(centerOdom);

      Point centerMap;
      centerMap.x = 0; //currentLocationMap.x + (1.0 * cos(currentLocationMap.theta));
      centerMap.y = 0; //currentLocationMap.y + (1.0 * sin(currentLocationMap.theta));
...
currentLocationMap.x = (-1.0 * cos(currentLocationMap.theta));//SWRA3
currentLocationMap.y = (-1.0 * sin(currentLocationMap.theta));//SWRA3

initial.x = (-1.0 * cos(currentLocationMap.theta));//SWRA3
initial.y = (-1.0 * sin(currentLocationMap.theta));//SWRA3

currentLocation.x = -1.0 * cos(currentLocation.theta);//SWRA3
currentLocation.y = -1.0 * sin(currentLocation.theta);//SWRA3
  ...
  if (currentMode == 2 || currentMode == 3)
  {
    ...
    //SIPI - allows the swarmies to update when dropping
    if (result.broadcast)
    {
  ...
  obstaclePublisher.publish(collision_msg);
    ...
    if (sqrt(pow(saved_loc.x-currentLocation.x, 2) + pow(saved_loc.y-currentLocation.y, 2)) > 1 && unLocky < 3)//SWRA: Broadcast later here
  {
unLocky++;
positionPublisher->setDetections(tags, saved_loc);
   }
...
void targetHandler(const apriltags_ros::AprilTagDetectionArray::ConstPtr& message) {
  ...
  if (message->detections.size() > 0) {
    ...
    // To enable recruitment uncomment these lines.
     unLocky = 0; //SWRA
     saved_loc.x = currentLocationMap.x + initial.x;
     saved_loc.y = currentLocationMap.y + initial.y;
...
void publishStatusTimerEventHandler(const ros::TimerEvent&) {
  std_msgs::String msg;
  msg.data = "SWRA";		//change this with team name - I did
...
void recruitmentHandler(const swarmie_msgs::Recruitment& msg)
{
if(sqrt(pow(msg.x-currentLocation.x, 2) + pow(msg.y-currentLocation.y, 2)) < 5)//activate if less than 5m away
     { 
   if(msg.name.data != publishedName) {
      Point p;
      p.x = msg.x - initial.x;//SWRA3
      p.y = msg.y - initial.y;//SWRA3
...
//SIPI - everything below
void dropTrafficHandler(const std_msgs::Int8& msg)
{
  //Will update dropoffcontroller on current traffic
  if(traffic != msg.data) {
    traffic = msg.data;
    logicController.checkCenterTraffic(traffic);
    cout << "Received Traffic Update: " << traffic << endl;
  }
}

void updateTraffic( ) {
  //Will broadcast if told by the dropoffcontroller

    if(traffic < 0) {
      traffic = 0;
      cout << "Traffic Correction = low" << endl;
    }
/*
    if(traffic > 4) {
      traffic = 4;
      cout << "Traffic Correction = high" << endl;
    }
*/
    if(result.isDropping && trafficAdjustment != 1) {

      trafficAdjustment = 1;
      traffic_msg.data = traffic + trafficAdjustment;
      cout << "Broadcast: " << traffic << " + " << trafficAdjustment << endl;
    }
    else if(!result.isDropping && trafficAdjustment != -1) {
    ...
    dropTrafficPublisher.publish(traffic_msg);
